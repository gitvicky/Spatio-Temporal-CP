...
plt.plot(1-alpha_levels, emp_cov_res, label='Residual - weighted - known pdf' ,ls='-.', color='teal', alpha=0.8, linewidth=1.0)
plt.xlabel('1-alpha')
...
plt.figure()
plt.plot(1-alpha_levels, 1-alpha_levels, label='Ideal', color ='black', alpha=0.8, linewidth=1.0)
plt.plot(1-alpha_levels, emp_cov_res, label='Residual - weighted - Actual PDF' ,ls='-.', color='teal', alpha=0.8, linewidth=1.0)
...
# # %%
# ##############################################################################################################################################
# #Using Kernel Density Estimation using sklearn KDE - #https://scikit-learn.org/stable/modules/generated/sklearn.neighbors.KernelDensity.html#sklearn.neighbors.KernelDensity.score_samples
# ##############################################################################################################################################

# # %% 
# from sklearn.neighbors import KernelDensity
# def likelihood_ratio_KDE(x, kde1, kde2):
#     pdf1 = kde1.score_samples(np.expand_dims(x, -1))
#     pdf2 = kde2.score_samples(np.expand_dims(x, -1))
#     return pdf2 / pdf1 
...
# kde1 = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(np.expand_dims(x_calib, -1))
# kde2 = KernelDensity(kernel='gaussian', bandwidth=0.5).fit(np.expand_dims(x_shift, -1))
