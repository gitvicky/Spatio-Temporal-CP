...
print("###############################################################")
print("Propagating both calibration and prediction sets through FNO")
print("###############################################################")
...
t2 = default_timer()
print('Time used to evaluate:', t2-t1)

# cal_u = cal_u.numpy()
# cal_scores = np.max(np.abs(cal_u-cal_mean), axis = 0)

###
# Conformal with max residual


###
# Conformal with data std

# modulation_data = np.std(cal_u.numpy(), axis = 0)
modulation_err = np.std(cal_u.numpy() - cal_mean.numpy(), axis = 0)
# modulation_err = np.mean(np.abs(cal_u.numpy() - cal_mean.numpy()), axis = 0)

cal_scores = np.max(np.abs(cal_u.numpy()-cal_mean.numpy()), axis = (1,2,3,4))
# cal_scores_data = np.max(np.abs(cal_u.numpy()-cal_mean.numpy())/modulation_data, axis = (1,2,3,4))
cal_scores_err = np.max( np.abs((cal_u.numpy()-cal_mean.numpy())/modulation_err), axis = (1,2,3,4))

alpha = 0.2

qhat = np.quantile(cal_scores, np.ceil((n+1)*(1-alpha))/n, axis = 0, interpolation='higher')
qhat_err = np.quantile(cal_scores_err, np.ceil((n+1)*(1-alpha))/n, axis = 0, interpolation='higher')


prediction_sets_abs =  [mean.numpy() - qhat, mean.numpy() + qhat]
prediction_sets_err =  [mean.numpy() - qhat_err*modulation_err, mean.numpy() + qhat_err*modulation_err]
...
empirical_coverage = ((y_response >= prediction_sets_abs[0]).all(axis = (1,2,3,4)) & (y_response <= prediction_sets_abs[1]).all(axis = (1,2,3,4))).mean()
empirical_coverage_err = ((y_response >= prediction_sets_err[0]).all(axis = (1,2,3,4)) & (y_response <= prediction_sets_err[1]).all(axis = (1,2,3,4))).mean()

print(f"The empirical coverage after calibration (abs) is: {empirical_coverage}")
print(f"alpha is: {alpha}")
print(f"1 - alpha <= empirical coverage is {(1-alpha <= empirical_coverage)}")

print(f"The empirical coverage after calibration (err) is: {empirical_coverage_err}")
print(f"alpha is: {alpha}")
print(f"1 - alpha <= empirical coverage is {(1-alpha <= empirical_coverage_err)}")
...
plt.colorbar()
# %%
def calibrate(alpha, cal_mean):
    n = ncal
    y_response = pred_u.numpy()
        
    # cal_scores = np.max(np.abs(cal_u.numpy()-cal_mean), axis = (1,2,3,4))
    cal_scores = np.abs(cal_u.numpy()-cal_mean)
    qhat = np.quantile(cal_scores, np.ceil((n+1)*(1-alpha))/n, axis = 0, interpolation='higher')

    prediction_sets =  [mean.numpy() - qhat, mean.numpy() + qhat]
    empirical_coverage = ((y_response >= prediction_sets[0]).all(axis = (1,2,3,4)) & (y_response <= prediction_sets[1]).all(axis = (1,2,3,4))).mean()
...
for ii in tqdm(range(len(alpha_levels))):
    emp_cov.append(calibrate(alpha_levels[ii], cal_mean))
...
plt.savefig("MHD_comparison_pixelwise.png", bbox_inches='tight')
plt.show()
...
mean_vals = cal_mean
lower_vals = prediction_sets[0]
upper_vals = prediction_sets[1]
soln_vals = pred_u.numpy()
