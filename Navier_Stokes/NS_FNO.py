...
@author: vgopakum
FNO modelled over the unsteady laminar data built using OpenFOAM for exploring the ST-CP Framework. 
"""
# %%
...
run.init(folder="/Conformal_Prediction", tags=['Conformal Prediction', 'Navier-Stokes', 'arbitrary-grids', 'FNO'], metadata=configuration)
...
# %%
data_loc = '/home/ir-gopa2/rds/rds-ukaea-ap001/ir-gopa2/Code/simvue_testing/OpenFOAM' #CSD3
data_loc = os.getcwd() + '/Data' #Local
# data =  np.load(data_loc + '/turbulent/NS_turbulent.npz')
data =  np.load(data_loc + '/laminar/NS_laminar.npz') #Laminar Unsteady
# data =  np.load(data_loc + '/laminar_steady/NS_laminar_steady.npz')
# %%
ntrain = 45
ntest = 5
...
if field == 'U':
    ...
    u_sol = data['V'].astype(np.float32)[-5:]
if field == 'P':
    u_sol = data['P'].astype(np.float32)[-5:]
...
# %%
#Saving the Model
model_loc = file_loc + '/Models/FNO_NavierStokes_' + run.name + '.pth'
...
# %%
output_plot = file_loc + '/Plots/NavierStokes_' + configuration['Field'] + '_' + run.name + '.png'
